...
recommendResult.single <- function(tree.list, x.b){
  ...
  result.list <- traverseForest_CO(tree.list,x.b)
  out <- NULL
  treat <- NULL
...
#'
#' @examples
traverseForest_CO <- function(forest, x.b) {
  # Check root is a list of trees
  # Check x.b is one observation
  return(purrr::map_dfr(forest,traverseTree_CO,x.b=x.b))
}
...
#'
#' @examples
traverseTree_CO <- function(root, x.b){
  ...
  if(root$isLeaf)
    ...
    if((x.b)%*%split.comb>=split.value)
      return(traverseTree_CO(root$children[[ge.node.index]],x.b))
    else
      return(traverseTree_CO(root$children[[l.node.index]],x.b))
  }
}
