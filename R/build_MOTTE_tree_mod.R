...
#'
#' with(train.dat,
#'     build_MOTTE_tree_MOD(x.b, x.e, factor(treat), y.b, y.e,
...
build_MOTTE_tree_MOD <- function(x.b, x.e, treat, y.b, y.e,
                             nodesize, nsplits, left.out) {

  trt.lvl <- levels(treat)
  if(length(trt.lvl) != 2)
    stop("Error Message: trt.lvl !=2 in build_MOTTE_tree_MOD")
  ...
  S_x <- crossprod(.x.b)
  S_Dx <- crossprod(T.delta.x)
  S_Dy <- crossprod(T.delta.y)

  cal_half <- function(mat, inverse=F){
    eigen_mat <- eigen(mat)

    if(!inverse)
    return(
      tcrossprod(
        tcrossprod(eigen_mat$vectors, diag(sqrt(eigen_mat$values))),
        eigen_mat$vectors)
    )
    else
      return(
        tcrossprod(
          tcrossprod(eigen_mat$vectors, diag(1/sqrt(eigen_mat$values))),
          eigen_mat$vectors)
      )
  }
  ...
  Left.matrix <-
    rbind(
      cbind(.x.b %*% cal_half(S_x, inverse=T) ,matrix(0,nrow=n,ncol=p+q)),
      cbind(.x.b %*% cal_half(S_x, inverse=T),matrix(0,nrow=n,ncol=p+q)),
      cbind(matrix(0,nrow=n,ncol=p), T.delta.x  %*% cal_half(S_Dx, inverse=T), matrix(0,nrow=n,ncol=q))
    )

  Right.matrix <-
    rbind(
      cbind(matrix(0,nrow=n,ncol=2*p), T.delta.y %*% cal_half(S_Dy, inverse=T)),
      cbind(matrix(0,nrow=n,ncol=p), T.delta.x  %*% cal_half(S_Dx, inverse=T), matrix(0,nrow=n,ncol=q)),
      cbind(matrix(0,nrow=n,ncol=2*p), T.delta.y %*% cal_half(S_Dy, inverse=T))
    )
  ...
  x.loading <- cal_half(S_x, inverse=T) %*% cca.res$xcoef[1:p,1]
  y.loading <- cal_half(S_Dy, inverse=T) %*% cca.res$xcoef[((2*p+1):(2*p+q)), 1]
