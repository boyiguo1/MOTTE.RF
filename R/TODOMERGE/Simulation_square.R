...
Y.train.base <- (X.train.base)^2%*%Z + mvrnorm(n.train, rep(0,q), 0.1*diag(q))
Y.train.end <- (X.train.end)^2%*%Z + mvrnorm(n.train, rep(0,q), 0.1*diag(q))
...
Y.test.case.end <- (X.test.case.end)^2%*%Z
Y.test.control.end <- (X.test.control.end)^2%*%Z
