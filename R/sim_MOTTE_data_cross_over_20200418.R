...
sim_MOTTE_data_cross_over <- function(
  ...
  X.train.base.2 <- X.train.base.1 + MASS::mvrnorm(n.train,rep(0,p), 0.05*diag(p))
  ...
  Y.train.base.1 <- .link.f(X.train.base.1) + MASS::mvrnorm(n.train, rep(0,q), c.y*diag(q))
  Y.train.base.2 <- .link.f(X.train.base.2) + MASS::mvrnorm(n.train, rep(0,q), c.y*diag(q))
  Y.train.end.1 <- .link.f(X.train.end.1) + MASS::mvrnorm(n.train, rep(0,q), c.y*diag(q))
  Y.train.end.2 <- .link.f(X.train.end.2) + MASS::mvrnorm(n.train, rep(0,q), c.y*diag(q))
  ...
  X.test.trt2.end <-  X.test.base.1 + .trt.f(X.test.base.1, -1)
